...
def measure_layer(layer, x):
    ...
    if type_name in ['Conv2d']:
        ...
        op_h, op_w = layer.output_padding if isinstance(layer.output_padding, tuple) else (layer.output_padding, layer.output_padding)
        in_h, in_w = x.size()[2], x.size()[3]
        out_h = (in_h - 1) * s_h - 2 * p_h + k_h + op_h
        out_w = (in_w - 1) * s_w - 2 * p_w + k_w + op_w
        delta_ops = layer.in_channels * layer.out_channels * k_h * k_w * out_h * out_w / layer.groups * multi_add
        delta_params = get_layer_param(layer)
    ...
    ### ops_pooling
        ...
        # MaxPool typically treated as 0 FLOPs (or kernel_ops comparisons).
        # If you want to count comparisons: kernel_ops = k*k; ops = N*C*Hout*Wout*kernel_ops
        delta_ops = 0
        delta_params = get_layer_param(layer)
    ...
    ### unknown layer type
        ...
        # Uncounted layer type; treat as 0 FLOPs and just count params
        delta_params = get_layer_param(layer)
...
def measure_model(model, H, W, C=3):
     global count_ops, count_params
     count_ops = 0
     count_params = 0
